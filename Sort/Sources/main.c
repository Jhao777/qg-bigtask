...
/* void Add()
{
    ...
    while(1){
        ...
        {
            if(a[i]<=57&&a[i]>=48)                  //0~9的啊洗卡码是48~57
                j++;
        }
...
{
    //Add();                  //测试数据生成程序
    printf("注意！在macOS系统下单位为微秒,windows系统是毫秒!\n");
    Read();                   //读取测试数据并测试排序函数的程序

    int a[10000]={0};
    bigdata(a,10000);
    int b[50000]={0};
    bigdata(b,50000);             //大数据量与大量小数组的排序测试程序
    int static c[200000] ;
    bigdata(c,200000);
    smalldata();
